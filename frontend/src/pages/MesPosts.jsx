...
const MesPosts = () => {
  ...
  const handleComment = async (index) => {
    ...
    if (comment && comment.trim() !== '') {
      ...
      dispatch({ type: 'SET_POSTS', payload: newPosts });
      localStorage.setItem('posts', JSON.stringify(newPosts));
      dispatch({ type: 'SET_CURRENT_COMMENTS', payload: { ...state.currentComments, [index]: '' } });
      ...
      >
        ...
        {state.posts.length === 0 ? (
            ...
            >
                ...
                <h2 className="text-2xl font-bold" style={{ fontFamily: 'Georgia, serif', color: '#4a5568' }}>{post.post}</h2>
